...
const HDC device = GetDC(GetConsoleWindow());
	...
	for (int i = 0; i < 5; i++)
		for (int j = 0; j < 5; j++)
		{
			...
			for (int k = 0; k < 10; k++)
				if (list[k] != nullptr)
					if (brick[i][j].getHitPoints() > 0)
						if (list[k]->getLocation().y >= brick[i][j].getPosition().y  && list[k]->getLocation().y - 3 <= brick[i][j].getPosition().y + Brick::size.y)
